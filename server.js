...
// --- ЛОГИРОВАНИЕ ДЛЯ ПРОВЕРКИ (Смотри это в Logs на Render) ---
console.log("--- ПРОВЕРКА ОКРУЖЕНИЯ ---");
console.log("Текущий путь (__dirname):", __dirname);
try {
    const files = fs.readdirSync(__dirname);
    console.log("Файлы, которые видит сервер в корне:", files);
    
    // Если есть папки, заглянем в них
    files.forEach(file => {
        if (fs.lstatSync(path.join(__dirname, file)).isDirectory()) {
            console.log(`Содержимое папки ${file}:`, fs.readdirSync(path.join(__dirname, file)));
        }
    });
} catch (e) { console.log("Ошибка сканирования папок:", e); }
console.log("--------------------------");
...
// Умная функция поиска и отдачи файла
function serveHTML(res, name) {
    // Список мест для поиска
    const paths = [
        path.join(__dirname, name),
        path.join(__dirname, 'public', name),
        path.join(__dirname, 'greho-smeh', name), // на случай если папка так называется
        path.join(__dirname, 'src', name)
    ];

    for (let p of paths) {
        if (fs.existsSync(p)) {
            console.log(`Отдаю файл: ${p}`);
            return res.sendFile(p);
        }
    }

    // Если ничего не нашли, выводим список того, что есть в корне
    const available = fs.readdirSync(__dirname).join(', ');
    res.status(404).send(`
        <h1>Ошибка: Файл ${name} не найден!</h1>
        <p>Сервер видит только это: <b>${available}</b></p>
        <p>Убедись, что все HTML файлы лежат на GitHub рядом с server.js, а не в папках.</p>
    `);
}
...
async function getAIQuestion(isFinal = false) {
    try {
        const prompt = `Придумай один ${isFinal ? "вопрос на 3 ответа" : "смешной вопрос"} на русском. Юмор: мемный. Только текст.`;
