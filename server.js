...
io.on('connection', (socket) => {
    socket.on('create-room', () => {
        ...
        rooms[code] = { host: socket.id, players: [], pairs: [], currentPairIndex: 0, round: 1, bestAnswers: [] };
        socket.join(code);
    ...
    socket.on('join-room', ({ code, name }) => {
        if (rooms[code]) {
            ...
            io.to(rooms[code].host).emit('player-joined', name);
            socket.emit('joined-success');
        }
    });

    socket.on('start-game', (code) => setupRound(code, 1));
    ...
    function startNextPair(code) {
        ...
        if (!pair) {
            if (room.round === 1) setupRound(code, 2);
            else if (room.round === 2) setupRound(code, 3);
            else finishGame(code);
            return;
        }
        io.to(code).emit('round-started', { round: room.round, q: pair.q, p1_name: pair.p1.name, p2_name: pair.p2 ? pair.p2.name : "БОНУС" });
    }
    ...
    socket.on('submit-answer', ({ code, name, answers }) => {
        ...
        if (pair.ans1.length > 0 && (!pair.p2 || pair.ans2.length > 0)) {
            if (!pair.p2) {
                ...
                io.to(code).emit('show-voting', { round: room.round, q: pair.q, ans1: pair.ans1, ans2: pair.ans2, p1: pair.p1.name, p2: pair.p2.name });
            }
        }
    });
    ...
    socket.on('cast-vote', ({ code, voteNum, voterName }) => {
        ...
        if (pair.votes.length >= room.players.length - 2) {
            ...
            room.bestAnswers.push({ text: pair.ans1.join(", "), author: pair.p1.name, votes: v1 });
            room.bestAnswers.push({ text: pair.ans2.join(", "), author: pair.p2.name, votes: v2 });
            io.to(code).emit('voting-results', { pair });
