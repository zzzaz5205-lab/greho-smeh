...
io.on('connection', (socket) => {
    ...
    function showVoting(code, pair) {
        ...
        // ЕСЛИ СОЛО — авто-завершение через 10 секунд
        if (!pair.p2) {
            if(timers[code]) clearTimeout(timers[code]);
            timers[code] = setTimeout(() => {
                if (rooms[code]) finishPair(code);
            }, 10000);
        }
    }
    ...
    async function finishPair(code) {
        ...
        setTimeout(() => { 
            if (rooms[code]) { rooms[code].currentPairIndex++; sendPair(code); } 
        }, 8000);
    }
