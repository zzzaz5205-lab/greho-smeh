...
// ПРЯМЫЕ ПУТИ (Чтобы Render всегда находил файлы)
...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', ({ code, name }) => {
        ...
        // ЖЕСТКАЯ ОЧИСТКА КЛОНОВ ПО ИМЕНИ
    ...
    socket.on('start-game', (code) => { if (rooms[code] && rooms[code].players.length >= 2) { rooms[code].gameStarted = true; startRound(code, 1); } });
