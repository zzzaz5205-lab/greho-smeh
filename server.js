...
async function getProPrompt(room, isFinal = false) {
    ...
    let style = s.eighteenPlus ? "18+, токсичный, мемный" : "абсурдный, Jackbox style";
    try {
        const result = await aiModel.generateContent(`Придумай 1 ${isFinal ? "вопрос на 3 ответа" : "вопрос"} для игры Грехо-Смех. Стиль: ${style}. Только текст.`);
...
io.on('connection', (socket) => {
    ...
    socket.on('tutorial-finished', (code) => { startRound(code, 1); });
    ...
    function sendPair(code) {
        ...
        let time = room.settings.hellMode ? Math.floor(room.settings.timer/2) : room.settings.timer;
        io.to(code).emit('round-started', { round: room.round, q: pair.q, p1_name: pair.p1.name, p2_name: pair.p2?.name, time });
        if(timers[code]) clearTimeout(timers[code]);
        timers[code] = setTimeout(() => forceSubmit(code), (time + 1) * 1000);
    }
    ...
    function showVoting(code, p) {
        ...
        if (!p.p2) { // СОЛО: 10 секунд и дальше
            if(timers[code]) clearTimeout(timers[code]);
            timers[code] = setTimeout(() => finishPair(code), 10000);
        }
    }
