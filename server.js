...
async function getAIQuestion(isFinal = false) {
    try {
        const prompt = `Ты сценарист игры Смехлыст. Придумай один ${isFinal ? "вопрос требующий списка из 3 смешных вещей" : "смешной и абсурдный вопрос"} на русском. Юмор: мемный, острый. Выдай только текст вопроса.`;
        const result = await aiModel.generateContent(prompt);
        return result.response.text().trim();
    } catch (e) { return isFinal ? "3 причины не доверять Ванге" : "Почему vangavgav лысый?"; }
}

async function getAIComment(q, a1, a2) {
    try {
        const result = await aiModel.generateContent(`Вопрос: "${q}". Ответы игроков: "${a1}" и "${a2}". Напиши одну короткую (5 слов) издевательскую реакцию ведущего на русском.`);
        return result.response.text().trim();
    } catch (e) { return "Это было... странно."; }
}
...
io.on('connection', (socket) => {
    socket.on('create-room', (oldCode) => {
        ...
        if (!rooms[code]) {
            rooms[code] = { host: socket.id, players: [], round: 1, currentPairIndex: 0, pairs: [], gameStarted: false, modId: null, settings: { timer: 35, voice: 'male', bonusX2: true, moderation: false } };
    ...
    socket.on('join-room', ({ code, name }) => {
        ...
        r.players = r.players.filter(p => p.name !== name); // Убираем клона
        const p = { id: socket.id, name, char: null, score: 0, lastPoints: 0 };
        r.players.push(p);
    ...
    function sendPair(code) {
        ...
        io.to(code).emit('round-started', { q: p.q, p1: p.p1?.name, p2: p.p2?.name, isFinal: p.isFinal, round: r.round, settings: r.settings });
    ...
    function showVoting(code, p) {
        io.to(code).emit('show-voting', { ans1: p.ans1, ans2: p.ans2, isSolo: !p.p2, p1: p.p1?.name, p2: p.p2?.name });
    ...
    async function finishPair(code) {
        ...
        io.to(code).emit('voting-results', { v1, v2, p1: p.p1, p2: p.p2, aiComment: comm });
