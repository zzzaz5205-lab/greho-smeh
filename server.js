...
// --- ГЛАВНЫЙ ФИКС ПУТЕЙ ---
// Говорим серверу искать статику и в корне, и в папке public
app.use(express.static(path.join(__dirname)));
app.use(express.static(path.join(__dirname, 'public')));

// Явные маршруты для каждой страницы
app.get('/', (req, res) => {
...
app.get('/host', (req, res) => {
    res.sendFile(path.join(__dirname, 'host.html'), (err) => {
        if (err) res.sendFile(path.join(__dirname, 'public', 'host.html'));
    });
});

app.get('/player', (req, res) => {
    res.sendFile(path.join(__dirname, 'player.html'), (err) => {
        if (err) res.sendFile(path.join(__dirname, 'public', 'player.html'));
    });
});

app.get('/mod', (req, res) => {
    res.sendFile(path.join(__dirname, 'mod.html'), (err) => {
        if (err) res.sendFile(path.join(__dirname, 'public', 'mod.html'));
    });
});

// --- ДАЛЬШЕ ЛОГИКА ИГРЫ (БЕЗ ИЗМЕНЕНИЙ) ---
...
async function getAIQuestion(lang, isFinal = false) {
    try {
        const prompt = `Придумай один ${isFinal ? "вопрос на 3 ответа" : "смешной вопрос"} для игры Quiplash на русском. Юмор: мемный. Только текст.`;
...
async function getAIComment(q, a1, a2) {
    try {
        const result = await aiModel.generateContent(`Вопрос: "${q}". Ответы: "${a1}" и "${a2}". Напиши 1 короткую едкую реакцию.`);
        return result.response.text().trim();
...
io.on('connection', (socket) => {
    socket.on('create-room', (oldCode) => {
        ...
        if (!rooms[code]) {
            rooms[code] = { host: socket.id, players: [], round: 1, currentPairIndex: 0, pairs: [], gameStarted: false, settings: { timer: 30, voice: 'male' }, modId: null };
    ...
    function sendPair(code) {
        ...
        io.to(code).emit('round-started', { round: room.round, q: pair.q, p1_name: pair.p1.name, p2_name: pair.p2 ? pair.p2.name : null });
